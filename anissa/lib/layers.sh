...
layer_get_links() {
  ...
  if [[ -f "$links_file" ]]; then
    # Filtrer les lignes AUTOGENERATED, commentaires et lignes vides
    grep -v '^##' "$links_file" 2>/dev/null | grep -v '^#' | grep -v '^\s*$' || true
  fi
}
...
layer_get_url() {
  ...
  if [[ -f "$links_file" ]]; then
    ...
    local first_line
    first_line=$(grep -v '^##' "$links_file" 2>/dev/null | grep -v '^#' | grep -v '^\s*$' | head -1 || true)
    if [[ -n "$first_line" ]]; then
      # Extraire l'URL (après le = si format label=url, sinon la ligne entière)
      if [[ "$first_line" == *"="* ]]; then
        echo "${first_line#*=}"
      else
        echo "$first_line"
